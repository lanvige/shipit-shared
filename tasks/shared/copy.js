...
module.exports = function(gruntOrShipit) {
  var copy = function copy(item) {
    ...
    return init(shipit).then(function(shipit) {
      ...
      .then(function() {
        var cmd = sprintf('if ( ! [ -h "%(target)s" ] ); then rm -rf "%(target)s" 2> /dev/null; cp "%(source)s" "%(target)s"; fi', {
          source: source,
          target: target
        });
  ...
  var copyDirs = function copyDirs() {
    var shipit = utils.getShipit(gruntOrShipit);
  ...
  utils.registerTask(gruntOrShipit, 'shared:copy:dirs', copyDirs);
  utils.registerTask(gruntOrShipit, 'shared:copy:files', copyFiles);
  utils.registerTask(gruntOrShipit, 'shared:copy', [
    'shared:copy:dirs',
    'shared:copy:files'
  ]);
};
